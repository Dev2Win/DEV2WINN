import React from 'react';
import ChatMenu from '@/components/chat/ChatMenu';
import ChatboxHeader from '@/components/chat/ChatboxHeader';
import Input from '@/components/chat/Input';

const page = () => {
  return (
    <div className="2xl:mx-auto 2xl:max-w-[1400px] rounded-md">
      <div className="relative h-[78vh] bg-white shadow-md overflow-hidden">
        <h1 className='font-bold text-xl text-purple-1 px-4 py-2'>Messages</h1>
        <div className="grid grid-cols-12 gap-4 h-[90%] absolute left-0 right-0 ">
          <section className="overflow-y-scroll mb-4 scrollbar-hidden col-span-4">
             <ChatMenu/>
          </section>
          <section className="col-span-8 border border-gray-200 rounded-md mb-4 relative">
            <ChatboxHeader/>
            <Input/>
          </section>
        </div>
      </div>
    </div>
  );
};

export default page;
