...
const ChatRender = () => {
  ...
  useEffect(() => {
    ...
    socketInstance.on('conversation',(data)=>{
      ...
      const conversationUserData = data.map((conversationUser:any,index:any)=>{
          ...
          else if(conversationUser?.receiver?._id !==  user?.publicMetadata?.userId){
              return{
                  ...conversationUser,
  ...
  const filteredMessages = messages.filter(
    (msg:any) =>
      (msg.msgByUserId === user?.publicMetadata?.userId && msg.msgByUserId === selectedUser?._id) ||
      (msg.msgByUserId === selectedUser?._id && msg.msgByUserId === user?.publicMetadata?.userId)
  );
  ...
  return (
    <div className="2xl:mx-auto 2xl:max-w-[1400px] rounded-md">
      <div className="relative h-[78vh] bg-white shadow-md overflow-hidden">
        ...
        <div onClick={()=> setOpen(!open)} className="font-bold text-xl text-purple-1 w-[40px] h-[40px] mt-2 cursor-pointer flex items-center justify-center  rounded-full bg-purple-200">+</div>
        </div>
        <div className="grid lg:grid-cols-12 gap-4 h-[90%] absolute left-0 right-0 ">
          <section className="overflow-y-scroll mb-4 scrollbar-hidden lg:col-span-5 xl:col-span-4">
            <ChatMenu onSelectUser={handleSelectUser} users={allUsers} showChatbox={showChatbox} />
          </section>
            ...
            {selectedUser ? (
              <>
                ...
                <div className=' h-[290px] p-2 overflow-x-hidden bg-gray-50 overflow-y-scroll scrollbar space-y-3  '>
                  ...
                  <div className={` max-w-[200px] p-2 text-gray-600 text-[10px] rounded-lg flex items-baseline justify-between  ${user?.publicMetadata?.userId === msg?.msgByUserId ? "ml-auto bg-purple-50" : "bg-white"}`} key={index}> 
                    <p>{msg.text}</p> 
                    <div className=""> 
                      <p className=' text-purple-400 text-[8px] mt-2'>{msg?.seen ? 'seen': 'unseen'}</p>
                      <p className='text-[8px] ml-auto w-fit'>{moment(msg?.createdAt)?.format('hh:mm')}</p>
                    </div>
                  </div>
                  ))}
                </div>
      ...
      <MeetingModal isOpen={open} onClose={()=> setOpen(!open) } title="Connections">
         <div className="">
            {connections?.map((con:any)=>(
               <ChatMenuCard key={con?._id} user={con} setClose={()=> setOpen(!open)} onSelectUser={handleSelectUser}/> 
            ))}
          </div>
      </MeetingModal>
    </div>
