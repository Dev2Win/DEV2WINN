...
- [ ] FastAPI matching endpoint (TF-IDF + cosine + structured filters)
- [ ] Profile→document builder + embedding/vector pipeline
- [ ] Qdrant collection + upsert on profile change (via worker)
- [ ] Hybrid scoring + cold-start heuristics
- [ ] Recommendations API in `api` (cache in Redis) + dashboard wiring
- [ ] Feedback signal capture (accepted/dismissed matches)
...
- [ ] FastAPI Claude tool loop + tool registry (see docs/AI_AGENT.md)
- [ ] Tools: book_session, get_roadmap, lookup_mentor, summarize_progress, course_qa(RAG)
- [ ] RAG pipeline over course content (Qdrant)
- [ ] Token-efficiency: prompt caching, compaction, model routing, per-user budget
- [ ] SSE streaming chat UI in web
- [ ] Guardrails: safety, scope limits, escalation to human mentor
